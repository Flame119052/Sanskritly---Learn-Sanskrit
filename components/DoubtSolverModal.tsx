...
export const DoubtSolverModal: React.FC<DoubtSolverModalProps> = ({ isOpen, onClose, topic, files }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-950 z-50 flex flex-col animate-fade-in">
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          {messages.map((msg, index) => (
            <div key={index} className={`flex items-start gap-3 ${msg.isUser ? 'justify-end' : ''}`}>
              ...
              <div className={`max-w-2xl p-4 rounded-xl ${msg.isUser ? 'bg-violet-600 text-white' : 'bg-slate-800 text-slate-200'}`}>
      ...
      <footer className="p-4 border-t border-slate-800 bg-slate-900 flex-shrink-0">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center gap-3">
            <input
              ...
              className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-3 text-base text-white focus:ring-2 focus:ring-cyan-500 focus:outline-none"
              disabled={isLoading}
            />
            <button onClick={handleSend} disabled={isLoading || !userInput.trim()} className="bg-cyan-600 hover:bg-cyan-500 text-white p-3 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0">
